...
'''Module with python bindings for some FiniteFlow32 routines.

WARNING: This is a w.i.p. and the API is not stable yet.
'''


from _cffi_finiteflow32 import lib as _lib, ffi as _ffi
...
class Success:
    ...
    def __repr__(self):
        return 'finiteflow32.SUCCESS'


class FFlowError(Exception):
    def __init__(self):
        super().__init__("FiniteFlow32 detected an error "
                         "(check for any FF_LOG messages).")
    def __eq__(self, other):
        return type(self) is type(other)
    def __repr__(self):
        return 'finiteflow32.ERROR'
...
class MissingPoints(Exception):
    ...
    def __eq__(self, other):
        return type(self) is type(other)
    def __repr__(self):
        return 'finiteflow32.MISSING_POINTS'
...
class MissingPrimes(Exception):
    ...
    def __eq__(self, other):
        return type(self) is type(other)
    def __repr__(self):
        return 'finiteflow32.MISSING_PRIMES'
